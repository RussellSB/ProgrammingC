...
int insertPair(HashTable * hashTable,const char * key, const char * value){
    ...
    if(hashTable->bucket[hashValue]==NULL){//if bucket doesnt exist
        ...
        }else{ //if c is equal to the initial max collisions, no space for new pair, bucket needs to dynamically grow

            //reallocate to add one new column to the bucket for the new pair
            hashTable->bucket[hashValue] = realloc(hashTable->bucket[hashValue],sizeof(Bucket)*hashTable->bucket[hashValue]->max_c+1);
